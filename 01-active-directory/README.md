# Lab 01 — Active Directory: Provisionamento & Monitoramento
...
Nova contratação - > Identidade -> Conta no Active Directory - > Departamento -> Financeiro - > GRP_FIN_Modificacao -> Acesso aos recursos
...
Get-ADUser -Filter "EmployeeID -eq 'EMP002'" -Properties EmployeeID | Select-Object Name,SamAccountName,EmployeeID
...
EmployeeID = EMP002  - >  IGA  -> Personnel Number = EMP002 -> Active Directory -> EmployeeID = EMP002
...
IAM cria identidade - > Senha inicial definida - > pwdLastSet = 0 - >  Primeiro logon - > Usuário altera a senha -> PasswordLastSet atualizado
...
Subject - > realizou a criação - > New Account
...
LAB\\Administrator - >  criou - > LAB\\ana.souza
...
Successful Logon - > Logon ID - > 4720 User Account Created
...
Department = Financeiro - > Account = Enabled -> GRP_FIN_Modificacao
...
Add-ADGroupMember -Identity "GRP\_FIN\_Modificacao" -Members "ana.souza"
...
Subject - > adicionou - > Member - >  ao - > Group
...
Administrator - > Ana Souza - > GRP_FIN_Modificacao
...
Ana Souza - > Department = Financeiro - > GRP_FIN_Modificacao - > Acesso compatível com a função
...
Usuário comum - > Domain Admins - > Alteração inesperada - > INVESTIGAR
...
IAM busca responder:  O usuário deveria possuir esse acesso?

Blue Team busca responder: A alteração foi legítima e realizada da maneira esperada?
...
Quem possui acesso? - >  A qual recurso? - > Por qual motivo? - > Quem concedeu? - > A concessão foi autorizada? - > O acesso ainda é necessário?
